...
Public Class Core
    ...
    Public Shared Function TM(TMCommand As String) As String
        ...
        Try
            ...
            'Get the Balances
            Dim OriginBalance As Long = ReadFromFile(UMSWEBDir & "\SSH\USERS\" & UserID & "\" & OriginBank & "\Balance.dll")
            Dim DestinationBalance As Long = ReadFromFile(UMSWEBDir & "\SSH\USERS\" & UserID & "\" & DestinationBank & "\Balance.dll")
            ...
            'Write Balances
            ToFile(UMSWEBDir & "\SSH\USERS\" & UserID & "\" & OriginBank & "\balance.dll", OriginBalance)
            ToFile(UMSWEBDir & "\SSH\USERS\" & UserID & "\" & DestinationBank & "\balance.dll", DestinationBalance)

            'Logging
            AddToFile(UMSWEBDir & "\SSH\users\" & UserID & "\" & OriginBank & "\log.log", "[" & DateTime.Now & "] You ~vibed~ " & TransferAmount.ToString("N0") & "p to " & DestinationBank)
            AddToFile(UMSWEBDir & "\SSH\users\" & UserID & "\" & DestinationBank & "\log.log", "[" & DateTime.Now & "] You ~vibed~ " & TransferAmount.ToString("N0") & "p from " & OriginBank)
    ...
    Public Shared Function INFO(UserID As String)
        ...
        If File.Exists(UMSWEBDir & "\SSH\USERS\" & UserID & "\UMSNB\Balance.dll") Then
            HasUMSNB = 1
            UMSNBBalance = ReadFromFile(UMSWEBDir & "\SSH\USERS\" & UserID & "\UMSNB\Balance.dll")
        End If

        If File.Exists(UMSWEBDir & "\SSH\USERS\" & UserID & "\GBANK\Balance.dll") Then
            HasGBANK = 1
            GBANKBalance = ReadFromFile(UMSWEBDir & "\SSH\USERS\" & UserID & "\GBANK\Balance.dll")
        End If

        If File.Exists(UMSWEBDir & "\SSH\USERS\" & UserID & "\RIVER\Balance.dll") Then
            HasRIVER = 1
            RIVERBalance = ReadFromFile(UMSWEBDir & "\SSH\USERS\" & UserID & "\RIVER\Balance.dll")
        End If
        ...
        Try
            notifs = File.ReadAllLines(UMSWEBDir & "\SSH\USERS\" & UserID & "\notifs.txt").Length
    ...
    Public Shared Function GetDirectory() As String
        ...
        FileOpen(1, String.Concat(UMSWEBDir, "\SSH\INCOMEMAN\Corporate.isf"), OpenMode.Input)
        While Not EOF(1)
            TempReadString = LineInput(1)
            If (TempReadString.StartsWith("USER")) Then
                ReDim Preserve directoryarray(NumberOfUsers - 1)
        ...
        NTNum = 1
        FileOpen(1, String.Concat(UMSWEBDir, "\SSH\INCOMEMAN\NonTaxed.isf"), OpenMode.Input)
        While Not EOF(1)
...
RedoIDGEN:
            ...
            If RegNAME.EndsWith(" (Corp.)") Then
                ...
                FileOpen(1, String.Concat(UMSWEBDir, "\SSH\INCOMEMAN\Corporate.isf"), OpenMode.Input, OpenAccess.[Default], OpenShare.[Default], -1)
                Dim N As Integer = 1
                While Not EOF(1)
                    str14 = LineInput(1)
            ...
            Else
                ...
                FileOpen(1, String.Concat(UMSWEBDir, "\SSH\INCOMEMAN\UserList.isf"), OpenMode.Input, OpenAccess.[Default], OpenShare.[Default], -1)
                Dim N As Integer = 1
                While Not EOF(1)
                    str14 = LineInput(1)
                    If (str14.StartsWith("USER")) Then
    ...
    Public Shared Function Certify(Transaction As String) As String
        Try
            AddToFile(WEBDir & "\Certifications.log", "{Certified on: " & DateTime.Now.ToString & "} " & Transaction)
        Catch ex As Exception
    ...
    Private Shared Function GetUsername(User As String) As String
        ...
        Return ReadFromFile(UMSWEBDir & "\SSH\USERS\" & User & "\NAME.DLL")
    End Function
    ''' <summary>
